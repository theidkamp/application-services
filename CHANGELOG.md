...
- Added `Store.encrypt_string()` / `Store.decrypt_string()`: string crypto under the store's own key, drop-in shaped like the key-based namespace functions minus the key parameter. These supersede `encrypt_string(key, ...)` / `decrypt_string(key, ...)`, which remain until Android and iOS have migrated (FXCM-2282).
